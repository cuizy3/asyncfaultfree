...
\item
In the second case where $set^A=reset^A \neq c1^A$, and $c1^A$ transitions, then in $s_{k+1}$, the only changes in the wires for $w^A$ are that $c1^A\neq c2^A$ and $set^A=reset^A = c1^A$, which satisfies the second induction hypothesis for wire $w$.  Since the transition occured on an intermediate wire then by similar reasoning to the first case, both induction hypothesis are true for any transition from this case. %don't know if this needs to be explained more (and add that the non-excited wires are still non-excited?)
